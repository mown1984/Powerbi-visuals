...
!function(powerbi) {
    ...
    !function(data) {
        var QueryProjectionCollection = function() {
            function QueryProjectionCollection(items, activeProjectionRef, showAll) {
                this.items = items, this._activeProjectionRef = activeProjectionRef, this._showAll = showAll;
            }
            return QueryProjectionCollection.prototype.all = function() {
                return this.items;
            }, Object.defineProperty(QueryProjectionCollection.prototype, "activeProjectionQueryRef", {
                get: function() {
                    return this._activeProjectionRef;
                },
                set: function(value) {
                    var queryRefs = this.items.map(function(val) {
                        return val.queryRef;
                    });
                    _.contains(queryRefs, value) && (this._activeProjectionRef = value);
                },
                enumerable: !0,
                configurable: !0
...
!function(powerbi) {
    ...
    !function(data) {
        ...
        !function(DataViewPivotCategoricalToPrimaryGroups) {
            ...
            function projectionsFromSelects(selects) {
            ...
            function inferUnpivotTransform(selects, dataViewMappings, dataView) {
                ...
                var roleKinds = createRoleKindFromMetadata(selects, dataView.metadata), projections = projectionsFromSelects(selects);
                if (dataViewMappings = powerbi.DataViewAnalysis.chooseDataViewMappings(projections, dataViewMappings, roleKinds), 
    ...
    !function(data) {
        ...
        !function(DataViewTransform) {
            function apply(options) {
                ...
                if (_.isEmpty(splits)) return [ transformDataView(prototype, objectDescriptors, dataViewMappings, transforms, colorAllocatorFactory, dataRoles) ];
                for (var transformedDataviews = [], _i = 0; _i < splits.length; _i++) {
            ...
            function transformDataView(prototype, objectDescriptors, roleMappings, transforms, colorAllocatorFactory, dataRoles, selectsToInclude) {
                ...
                return transformed.metadata = inherit(prototype.metadata), transformed = transformSelects(transformed, roleMappings, transforms.selects, transforms.projectionOrdering, selectsToInclude), 
            ...
            function applyTransformsToColumns(prototypeColumns, selects, rewrites) {
                ...
                for (var columns = inherit(prototypeColumns), i = 0, len = prototypeColumns.length; len > i; i++) {
                    ...
                    if (select) {
                        ...
                        select.sort && (column.sort = select.sort), rewrites.push({
            ...
            function createRuleEvaluationInstance(dataView, colorAllocatorFactory, ruleDesc, objectName, propertyValue, ruleType) {
                ...
                if (ruleOutput) {
                    ...
                    if (selectorToCreate) return ruleType.fillRule ? createRuleEvaluationInstanceFillRule(dataView, colorAllocatorFactory, ruleDesc, selectorToCreate, objectName, propertyValue) : ruleType.filter ? createRuleEvaluationInstanceFilter(dataView, ruleDesc, selectorToCreate, objectName, propertyValue) : void 0;
                }
            }
            ...
            function createRuleEvaluationInstanceFilter(dataView, ruleDesc, selectorToCreate, objectName, propertyValue) {
                if (dataView.categorical && dataView.categorical.categories && 1 === dataView.categorical.categories.length) {
                    ...
                    if (identityFields) {
                        ...
                        if (scopeIds) {
                            var rule = new data.FilterRuleEvaluation(scopeIds), properties = {};
                            return properties[ruleDesc.output.property] = rule, {
                                selector: selectorToCreate,
                                rules: [ rule ],
                                objects: [ {
                                    name: objectName,
            ...
            function evaluateDataRepetition(dataView, targetDataViewKinds, objectDescriptors, selector, rules, objectDefns) {
            ...
            function evaluateDataRepetitionCategoricalCategory(dataViewCategorical, objectDescriptors, selector, rules, containsWildcard, objectDefns) {
                if (dataViewCategorical.categories && 0 !== dataViewCategorical.categories.length) {
                    ...
                    if (targetColumn) {
                        var foundMatch, identities = targetColumn.identities, evalContext = data.createCategoricalEvalContext(dataViewCategorical, identities);
                        if (identities) {
            ...
            function createTransformActions(queryMetadata, visualElements, objectDescs, objectDefns) {
                if (queryMetadata && !_.isEmpty(queryMetadata.Select) || !_.isEmpty(visualElements) || objectDefns) {
                    ...
                    if (queryMetadata) {
                        ...
                        if (querySelects) for (var transformSelects_1 = transforms.selects = [], i = 0, len = querySelects.length; len > i; i++) {
                            var selectMetadata = querySelects[i], selectTransform = toTransformSelect(selectMetadata, i);
                            if (transformSelects_1.push(selectTransform), selectTransform.format && objectDescs) {
                                var formatStringProp = data.DataViewObjectDescriptors.findFormatString(objectDescs);
                                formatStringProp && (objectDefns || (objectDefns = {}), data.DataViewObjectDefinitions.ensure(objectDefns, formatStringProp.objectName, {
                    ...
                    if (visualElements) {
                        ...
                        for (var i = 0; visualElementsLength > i; i++) {
                            ...
                            visualElement.DataRoles && (transforms.selects || (transforms.selects = []), transforms.projectionOrdering || (transforms.projectionOrdering = {}), 
            ...
            function describeDataType(type, category) {
                type = type || data.SemanticType.None;
                var primitiveType = powerbi.PrimitiveType.Null;
                switch (type) {
                  case data.SemanticType.String:
                    primitiveType = powerbi.PrimitiveType.Text;
                    break;

                  case data.SemanticType.Number:
                    primitiveType = powerbi.PrimitiveType.Double;
                    break;

                  case data.SemanticType.Integer:
                    primitiveType = powerbi.PrimitiveType.Integer;
                    break;

                  case data.SemanticType.Boolean:
                    primitiveType = powerbi.PrimitiveType.Boolean;
                    break;

                  case data.SemanticType.Date:
                    primitiveType = powerbi.PrimitiveType.Date;
                    break;

                  case data.SemanticType.DateTime:
                    primitiveType = powerbi.PrimitiveType.DateTime;
                    break;

                  case data.SemanticType.Time:
                    primitiveType = powerbi.PrimitiveType.Time;
                    break;

                  case data.SemanticType.Year:
                    primitiveType = powerbi.PrimitiveType.Integer, category = "Year";
                    break;

                  case data.SemanticType.Month:
                    primitiveType = powerbi.PrimitiveType.Integer, category = "Month";
                }
            ...
            function populateDataRoles(roles, selects, projectionOrdering) {
                for (var i = 0, len = roles.length; len > i; i++) {
            ...
            function fillArray(selects, length) {
                for (var i = selects.length; length > i; i++) selects[i] = {};
            }
            function populateSplit(roles) {
                var selects = {}, split = {
                    selects: selects
                };
                if (roles) for (var i = 0, len = roles.length; len > i; i++) {
                    var role = roles[i];
            ...
            DataViewTransform.apply = apply, DataViewTransform.upgradeSettingsToObjects = upgradeSettingsToObjects, 
            DataViewTransform.createTransformActions = createTransformActions, DataViewTransform.createValueColumns = createValueColumns;
    ...
    !function(data) {
        ...
        !function(DataViewNormalizeValues) {
            ...
            function filterVariantMeasuresTreeNode(node, columnFilter, valueFilter) {
                if (node.values) for (var valueSourceIndex in node.values) columnFilter(valueSourceIndex) && ("object" == typeof node.values[valueSourceIndex] && "value" in node.values[valueSourceIndex] ? node.values[valueSourceIndex] = normalizeVariant(node.values[valueSourceIndex], "value", valueSourceIndex, valueFilter) : node.values = normalizeVariant(node.values, valueSourceIndex, valueSourceIndex, valueFilter)); else if (node.children) for (var _i = 0, _a = node.children; _i < _a.length; _i++) {
                    var child = _a[_i];
            ...
            function filterVariantMeasuresMatrix(dataview, columnFilter, valueFilter) {
                ...
                if (root) {
                    var adjustedColumnFilter = function(index) {
                        return index = toMetadataColumnIndex(dataview.valueSources, index), index ? columnFilter(index) : !1;
                    }, adjustedValueFilter = function(index, value) {
                        return index = toMetadataColumnIndex(dataview.valueSources, index), index ? valueFilter(index, value) : !0;
                    };
            ...
            function toMetadataColumnIndex(valueSources, valueSourceIndex) {
                var valueSource = valueSources[valueSourceIndex];
    ...
    !function(data) {
        ...
        !function(FieldExprPattern) {
            ...
            function getPropertyName(fieldExpr) {
                var column = fieldExpr.column || fieldExpr.columnAggr || fieldExpr.measure || fieldExpr.hierarchy;
            ...
            function toFieldExprEntityItemPattern(fieldExpr) {
                var field = fieldExpr.column || fieldExpr.columnAggr || fieldExpr.entityAggr || fieldExpr.hierarchy || fieldExpr.hierarchyLevel || fieldExpr.measure || fieldExpr.columnHierarchyLevelVariation && fieldExpr.columnHierarchyLevelVariation.source || fieldExpr;
                return {
                    schema: field.schema,
                    entity: field.entity,
...
!function(powerbi) {
    ...
    !function(DataViewAnalysis) {
        ...
        function supportsCategorical(dataView, categoryRoleMapping, usePreferredDataViewSchema) {
            ...
            if (categoryRoleMapping.rowCount) {
                ...
                rowCount) {
                    ...
                    !conformsToRange(len, rowCount)) return !1;
                }
            }
            return !0;
        }
        ...
        function conformsToRange(value, roleCondition, ignoreMin) {
            return roleCondition ? !ignoreMin && void 0 !== roleCondition.min && roleCondition.min > value ? !1 : void 0 !== roleCondition.max && roleCondition.max < value ? !1 : !0 : 0 === value;
        }
        function conformsToKind(roleCondition, roleName, projections, roleKindByQueryRef) {
            if (!roleCondition || void 0 === roleCondition.kind) return !0;
            ...
            if (roleCollection) for (var roleProjections = roleCollection.all(), _i = 0; _i < roleProjections.length; _i++) {
                ...
                if (roleKindByQueryRef[roleProjection.queryRef] !== expectedKind) return !1;
            }
            return !0;
        }
        function chooseDataViewMappings(projections, mappings, roleKindByQueryRef) {
            for (var supportedMappings = [], i = 0, len = mappings.length; len > i; i++) {
                var mapping = mappings[i], mappingConditions = mapping.conditions;
                if (mappingConditions && mappingConditions.length) for (var j = 0, jlen = mappingConditions.length; jlen > j; j++) {
                ...
                } else supportedMappings.push(mapping);
            }
            return ArrayExtensions.emptyToNull(supportedMappings);
        }
        function matchesCondition(projections, condition, roleKindByQueryRef) {
            for (var conditionRoles = Object.keys(condition), i = 0, len = conditionRoles.length; len > i; i++) {
                var roleName = conditionRoles[i], isDrillable = projections[roleName] && null != projections[roleName].activeProjectionQueryRef, roleCondition = condition[roleName], roleCount = getPropertyCount(roleName, projections, isDrillable);
                if (!conformsToRange(roleCount, roleCondition) || !conformsToKind(roleCondition, roleName, projections, roleKindByQueryRef)) return !1;
            }
            return !0;
        }
        ...
        DataViewAnalysis.conformsToRange = conformsToRange, DataViewAnalysis.chooseDataViewMappings = chooseDataViewMappings, 
...
!function(powerbi) {
    var data;
    !function(data) {
        function createCategoricalEvalContext(dataViewCategorical, identities) {
        ...
        var CategoricalEvalContext = function() {
            function CategoricalEvalContext(dataView, identities) {
                this.dataView = dataView, this.identities = identities, this.columnsByRole = {};
            }
            return CategoricalEvalContext.prototype.getCurrentIdentity = function() {
                var identities = this.identities, index = this.index;
                return identities && null != index ? identities[index] : void 0;
    ...
    !function(data) {
        var FilterRuleEvaluation = function(_super) {
            function FilterRuleEvaluation(scopeIds) {
                _super.call(this), this.selection = scopeIds;
            }
            return __extends(FilterRuleEvaluation, _super), FilterRuleEvaluation.prototype.evaluate = function(evalContext) {
                for (var currentScopeId = evalContext.getCurrentIdentity(), selectedScopeIds = this.selection.scopeIds, i = 0, len = selectedScopeIds.length; len > i; i++) if (powerbi.DataViewScopeIdentity.equals(currentScopeId, selectedScopeIds[i])) return !this.selection.isNot;
    ...
    !function(data) {
        var ArrayExtensions = jsCommon.ArrayExtensions, SQExprRewriter = function() {
            ...
            return SQExprRewriter.prototype.visitColumnRef = function(expr) {
                ...
                var origLeft = orig.left, rewrittenLeft = origLeft.accept(this), origRight = orig.right, rewrittenRight = origRight.accept(this);
                return origLeft === rewrittenLeft && origRight === rewrittenRight ? orig : new data.SQCompareExpr(orig.kind, rewrittenLeft, rewrittenRight);
        ...
        !function(EqualsToInRewriter) {
            ...
            var Rewriter = function(_super) {
                ...
                return __extends(Rewriter, _super), Rewriter.prototype.visitCompare = function(expr) {
                    if (expr.kind !== data.QueryComparisonKind.Equal) return this.visitUnsupported(expr);
    ...
    !function(data) {
        ...
        var FilterScopeIdsCollectorVisitor = function(_super) {
            ...
            return __extends(FilterScopeIdsCollectorVisitor, _super), FilterScopeIdsCollectorVisitor.prototype.getResult = function() {
                ...
                return null !== this.keyExprsCount ? this.unsupportedSQExpr() : (this.isRoot = !1, 
                expr.kind !== data.QueryComparisonKind.Equal ? this.unsupportedSQExpr() : expr.left.accept(this) && expr.right.accept(this));
            ...
            return __extends(FindComparandVisitor, _super), FindComparandVisitor.prototype.visitAnd = function(expr) {
                ...
                if (expr.kind === data.QueryComparisonKind.Equal) {
        ...
        !function(ScopeIdentityExtractor) {
            ...
            var ScopeIdExtractorImpl = function(_super) {
                ...
                return __extends(ScopeIdExtractorImpl, _super), ScopeIdExtractorImpl.prototype.visitAnd = function(expr) {
                    ...
                    return expr.kind !== data.QueryComparisonKind.Equal ? void this.visitDefault(expr) : (expr.left.accept(this), 
...
!function(powerbi) {
    ...
    !function(data) {
        ...
        var StringExtensions = jsCommon.StringExtensions, SQExpr = function() {
            ...
            return SQExpr.equals = function(x, y, ignoreCase) {
            ...
            }, SQExpr.prototype.accept = function(visitor, arg) {}, SQExpr.prototype.getMetadata = function(federatedSchema) {
                ...
                var field = data.SQExprConverter.asFieldPattern(this);
                if (field && field.hierarchyLevel) {
                    var fieldExprItem = data.FieldExprPattern.toFieldExprEntityItemPattern(field), hierarchy = federatedSchema.schema(fieldExprItem.schema).findHierarchy(fieldExprItem.entity, field.hierarchyLevel.name);
                    if (hierarchy) {
                        var hierarchyLevel = hierarchy.levels.withName(field.hierarchyLevel.level);
                ...
                var format = property.format, type = property.type, columnAggregate = field.columnAggr;
        ...
        var SQEntityExpr = function(_super) {
            function SQEntityExpr(schema, entity, variable) {
                _super.call(this), this.schema = schema, this.entity = entity, variable && (this.variable = variable);
            }
        ...
        var SQPropRefExpr = function(_super) {
            function SQPropRefExpr(source, ref) {
                _super.call(this), this.source = source, this.ref = ref;
            }
        ...
        var SQAggregationExpr = function(_super) {
            function SQAggregationExpr(arg, func) {
                _super.call(this), this.arg = arg, this.func = func;
            }
        ...
        var SQPropertyVariationSourceExpr = function(_super) {
            function SQPropertyVariationSourceExpr(arg, name, property) {
                _super.call(this), this.arg = arg, this.name = name, this.property = property;
            }
        ...
        var SQHierarchyExpr = function(_super) {
            function SQHierarchyExpr(arg, hierarchy) {
                _super.call(this), this.arg = arg, this.hierarchy = hierarchy;
            }
        ...
        var SQHierarchyLevelExpr = function(_super) {
            function SQHierarchyLevelExpr(arg, level) {
                _super.call(this), this.arg = arg, this.level = level;
            }
        ...
        var SQAndExpr = function(_super) {
            function SQAndExpr(left, right) {
                _super.call(this), this.left = left, this.right = right;
            }
        ...
        var SQBetweenExpr = function(_super) {
            function SQBetweenExpr(arg, lower, upper) {
                _super.call(this), this.arg = arg, this.lower = lower, this.upper = upper;
            }
        ...
        var SQInExpr = function(_super) {
            function SQInExpr(args, values) {
                _super.call(this), this.args = args, this.values = values;
            }
        ...
        var SQCompareExpr = function(_super) {
            function SQCompareExpr(kind, left, right) {
                _super.call(this), this.kind = kind, this.left = left, this.right = right;
            }
        ...
        var SQConstantExpr = function(_super) {
            function SQConstantExpr(type, value, valueEncoded) {
                _super.call(this), this.type = type, this.value = value, this.valueEncoded = valueEncoded;
            }
        ...
        var SQDateSpanExpr = function(_super) {
            function SQDateSpanExpr(unit, arg) {
                _super.call(this), this.unit = unit, this.arg = arg;
            }
        ...
        var SQDateAddExpr = function(_super) {
            function SQDateAddExpr(unit, amount, arg) {
                _super.call(this), this.unit = unit, this.arg = arg, this.amount = amount;
            }
        ...
        var SQExprEqualityVisitor = function() {
            ...
            return __extends(SQExprValidationVisitor, _super), SQExprValidationVisitor.prototype.visitIn = function(expr) {
                ...
                if (columnRefExpr) {
                    var supportedFuncs = data.SQExprUtils.getSupportedAggregates(columnRefExpr, this.schema);
                    supportedFuncs.indexOf(expr.func) < 0 && this.register(0);
                }
                ...
                return left instanceof SQColumnRefExpr ? (this.visitColumnRef(left), right instanceof SQConstantExpr && right.type.text ? this.validateCompatibleType(left, right) : this.register(6)) : this.register(5), 
                expr;
                ...
                leftType && rightType && !leftType.isCompatibleFrom(rightType) && this.register(7);
                ...
                if (fieldExpr.hierarchyLevel) {
                    var hierarchyLevelConceptualProperty = SQHierarchyExprUtils.getConceptualHierarchyLevelFromExpr(this.schema, fieldExpr);
        ...
        var SQHierarchyExprUtils, SQExprAggregateInfoVisitor = function(_super) {
        ...
        !function(SQHierarchyExprUtils) {
            function getConceptualHierarchyLevelFromExpr(conceptualSchema, fieldExpr) {
                var fieldExprItem = data.FieldExprPattern.toFieldExprEntityItemPattern(fieldExpr);
                return fieldExpr.hierarchyLevel ? SQHierarchyExprUtils.getConceptualHierarchyLevel(conceptualSchema, fieldExprItem.schema, fieldExprItem.entity, fieldExpr.hierarchyLevel.name, fieldExpr.hierarchyLevel.level) : void 0;
            }
            function getConceptualHierarchyLevel(conceptualSchema, schemaName, entity, hierarchy, hierarchyLevel) {
                var schema = conceptualSchema.schema(schemaName), hierarchyLevelColumn = schema.findHierarchy(entity, hierarchy);
                return hierarchyLevelColumn ? hierarchyLevelColumn.levels.withName(hierarchyLevel) : void 0;
            }
        ...
        }(SQHierarchyExprUtils || (SQHierarchyExprUtils = {}));
        var SQExprColumnRefInfoVisitor = function(_super) {
            ...
            return __extends(SQExprColumnRefInfoVisitor, _super), SQExprColumnRefInfoVisitor.prototype.visitColumnRef = function(expr) {
                ...
                if (hierarchy && hierarchy.arg instanceof SQPropertyVariationSourceExpr) {
                    ...
                    var entityExpr = hierarchy.arg, hierarchyLevelRef = SQHierarchyExprUtils.getConceptualHierarchyLevel(this.schema, entityExpr.schema, entityExpr.entity, hierarchy.hierarchy, expr.level);
    ...
    !function(data) {
        ...
        !function(SQExprUtils) {
            function getSupportedAggregates(expr, schema) {
                ...
                return valueType.text || valueType.bool || valueType.dateTime ? isPropertyIdentity ? [ Agg.CountNonNull ] : [ Agg.Count, Agg.CountNonNull ] : emptyList;
            }
            ...
            function getKpiStatus(expr, schema) {
                var kpi = getKpiStatusProperty(expr, schema);
                if (kpi) {
                    var measureExpr = data.SQExprConverter.asFieldPattern(expr).measure;
                    if (measureExpr) return data.SQExprBuilder.fieldExpr({
                        measure: {
            ...
            function getKpiMetadata(expr, schema) {
                ...
                if (property) {
                    if (property.measure && property.measure.kpi) return property.measure.kpi.statusMetadata;
                    var kpiStatusProperty = getKpiStatusProperty(expr, schema);
                    return kpiStatusProperty ? kpiStatusProperty.kpi.measure.kpi.statusMetadata : void 0;
                }
            }
            function getKpiStatusProperty(expr, schema) {
                var property = expr.getConceptualProperty(schema);
                if (property) {
                    var kpi = property.kpi;
                    return kpi && kpi.measure.kpi.status === property ? property : void 0;
                }
            }
                    ...
                    if (sqField) {
                        ...
                        if (column) {
                            if (schema.schema(column.schema) && sqField.column.name) {
                                ...
                                if (property && property.column) return property.column.defaultValue;
                            }
                        } else {
                            ...
                            if (hierarchyLevelField) {
                                ...
                                if (schema.schema(schemaName)) {
                                    ...
                                    if (hierarchy) {
                                        ...
                                        if (hierarchyLevel && hierarchyLevel.column && hierarchyLevel.column.column) return hierarchyLevel.column.column.defaultValue;
                                    }
                                }
                            }
                        }
            ...
            SQExprUtils.discourageAggregation = discourageAggregation, SQExprUtils.getSchemaCapabilities = getSchemaCapabilities, 
            SQExprUtils.getKpiStatus = getKpiStatus, SQExprUtils.getKpiMetadata = getKpiMetadata, 
            ...
            var SQExprDefaultNameGenerator = function(_super) {
                ...
                return __extends(IsDefaultValueVisitor, _super), IsDefaultValueVisitor.prototype.visitCompare = function(expr) {
                    return expr.kind !== data.QueryComparisonKind.Equal ? !1 : expr.right.accept(this);
    ...
    !function(data) {
        var SemanticQueryRewriter = function() {
            ...
            return SemanticQueryRewriter.prototype.rewriteFrom = function(fromValue) {
                ...
                for (var select = [], i = 0, len = selectItems.length; len > i; i++) {
    ...
    !function(data) {
        var ArrayExtensions = jsCommon.ArrayExtensions, SemanticQuery = function() {
            ...
            return SemanticQuery.create = function() {
                return SemanticQuery.empty || (SemanticQuery.empty = new SemanticQuery(new SQFrom(), null, null, [])), 
                SemanticQuery.empty;
                ...
                return 0 === arguments.length ? this.getSelect() : this.setSelect(values);
            }, SemanticQuery.prototype.getSelect = function() {
                return ArrayExtensions.extendWithName(_.map(this.selectItems, function(s) {
                    return {
                        name: s.name,
                        expr: s.expr
                    };
                ...
                for (var selectItems = [], from = this.fromValue.clone(), i = 0, len = values.length; len > i; i++) {
                    var value = values[i];
                    selectItems.push({
                        name: value.name,
                ...
                if (!(index >= this.selectItems.length)) {
                    ...
                    }, SemanticQuery.createWithTrimmedFrom(from, this.whereItems, this.orderByItems, select);
                }
                ...
                return selectItems.push({
                    ...
                    expr: SQExprRewriterWithSourceRenames.rewrite(expr, from)
                }), SemanticQuery.createWithTrimmedFrom(from, this.whereItems, this.orderByItems, selectItems);
            }, SemanticQuery.prototype.orderBy = function(values) {
                return 0 === arguments.length ? this.getOrderBy() : this.setOrderBy(values);
                ...
                return 0 === arguments.length ? this.getWhere() : this.setWhere(values);
                ...
                var rewriter = new data.SemanticQueryRewriter(exprRewriter), from = rewriter.rewriteFrom(this.fromValue), where = rewriter.rewriteWhere(this.whereItems, from), orderBy = rewriter.rewriteOrderBy(this.orderByItems, from), select = rewriter.rewriteSelect(this.selectItems, from);
...
!function(powerbi) {
    ...
    !function(data) {
        ...
        function getExprValue(expr, selectTransforms, table) {
            var rows = table.rows;
            if (!rows || 1 === rows.length) for (var cols = table.columns, selectIdx = 0, selectLen = selectTransforms.length; selectLen > selectIdx; selectIdx++) {
                var selectTransform = selectTransforms[selectIdx];
                if (SQExpr.equals(selectTransform.expr, expr) && selectTransform.queryName) for (var colIdx = 0, colLen = cols.length; colLen > colIdx; colIdx++) if (selectIdx === cols[colIdx].index) return rows[0][colIdx];
            }
        }
        ...
        var StaticEvalContext = function() {
            ...
            return StaticEvalContext.prototype.getExprValue = function(expr) {
                ...
                return dataView && dataView.table && selectTransforms ? getExprValue(expr, selectTransforms, dataView.table) : void 0;
            }, StaticEvalContext.prototype.getCurrentIdentity = function() {}, StaticEvalContext.prototype.getRoleValue = function(roleName) {}, 
            StaticEvalContext;
        }();
    }(data = powerbi.data || (powerbi.data = {}));
}(powerbi || (powerbi = {}));
