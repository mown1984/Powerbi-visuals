        ...
        var DataViewCategoricalReader = function() {
            ...
            return DataViewCategoricalReader.prototype.hasCategories = function() {
                ...
                if (void 0 === seriesIndex && (seriesIndex = 0), this.hasValues(roleName)) {
                    for (var valuesInRole = [], roleValueIndex = void 0, roleValueCount = this.valueRoleIndexMapping[roleName].length; roleValueCount > roleValueIndex; roleValueIndex++) valuesInRole.push(this.getValueInternal(roleName, categoryIndex, seriesIndex, roleValueIndex, !1));
                    return valuesInRole;
                }
            }, DataViewCategoricalReader.prototype.getAllHighlightsForRole = function(roleName, categoryIndex, seriesIndex) {
                if (void 0 === seriesIndex && (seriesIndex = 0), this.hasValues(roleName)) {
                    for (var valuesInRole = [], roleValueIndex = void 0, roleValueCount = this.valueRoleIndexMapping[roleName].length; roleValueCount > roleValueIndex; roleValueIndex++) valuesInRole.push(this.getValueInternal(roleName, categoryIndex, seriesIndex, roleValueIndex, !0));
                    return valuesInRole;
                }
                ...
                return this.hasAnyValidValues ? this.dataHasDynamicSeries ? this.grouped.length : valueRoleName ? this.valueRoleIndexMapping[valueRoleName].length : 1 : void 0;
...
!function(powerbi) {
    ...
    !function(data) {
        ...
        function getExprValueFromTable(expr, selectTransforms, table, rowIdx) {
            var rows = table.rows;
            if (!(_.isEmpty(rows) || rows.length <= rowIdx)) for (var targetExpr = getTargetExpr(expr, selectTransforms), cols = table.columns, selectIdx = 0, selectLen = selectTransforms.length; selectLen > selectIdx; selectIdx++) {
                var selectTransform = selectTransforms[selectIdx];
                if (SQExpr.equals(selectTransform.expr, targetExpr) && selectTransform.queryName) for (var colIdx = 0, colLen = cols.length; colLen > colIdx; colIdx++) if (selectIdx === cols[colIdx].index) return rows[rowIdx][colIdx];
            }
        }
        function getTargetExpr(expr, selectTransforms) {
            if (SQExpr.isSelectRef(expr)) for (var _i = 0, selectTransforms_2 = selectTransforms; _i < selectTransforms_2.length; _i++) {
                var selectTransform = selectTransforms_2[_i];
                if (selectTransform.queryName === expr.expressionName) return selectTransform.expr;
            }
            return expr;
        }
