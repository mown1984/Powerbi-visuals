        ...
        var DataViewCategoricalReader = function() {
            function DataViewCategoricalReader(dataView) {
                ...
                if (categorical && (values = categorical.values), this.hasAnyValidValues = !1, null != values) {
                    ...
                    grouped.length > 0 && (this.hasAnyValidValues = !0, this.grouped = grouped);
                }
            ...
            return DataViewCategoricalReader.prototype.hasCategories = function() {
                ...
                var measureIndex = this.getMeasureIndex(roleName), grouped = this.grouped;
                return this.hasAnyValidValues && -1 !== measureIndex ? this.dataHasDynamicSeries || 0 === seriesIndex ? grouped[seriesIndex].values[measureIndex].values : grouped[0].values[seriesIndex].values : void 0;
...
!function(powerbi) {
    ...
    !function(data) {
        ...
        !function(SQExprBuilder) {
            ...
            var FieldExprToSQExprVisitor = function() {
                ...
                return FieldExprToSQExprVisitor.prototype.visitColumn = function(column) {
                    return SQExprBuilder.columnRef(this.visitEntity(column), column.name);
                }, FieldExprToSQExprVisitor.prototype.visitColumnAggr = function(columnAggr) {
                    return SQExprBuilder.aggregate(this.visitColumn(columnAggr), columnAggr.aggregate);
                    ...
                    return SQExprBuilder.aggregate(this.visitEntity(entityAggr), entityAggr.aggregate);
                }, FieldExprToSQExprVisitor.prototype.visitHierarchy = function(hierarchyPattern) {
                    return SQExprBuilder.hierarchy(this.visitEntity(hierarchyPattern), hierarchyPattern.name);
                }, FieldExprToSQExprVisitor.prototype.visitHierarchyLevel = function(level) {
                    return SQExprBuilder.hierarchyLevel(this.visitHierarchy(level), level.level);
                }, FieldExprToSQExprVisitor.prototype.visitHierarchyLevelAggr = function(hierarchyLevelAggr) {
                    return SQExprBuilder.aggregate(this.visitHierarchyLevel(hierarchyLevelAggr), hierarchyLevelAggr.aggregate);
        ...
        !function(FieldExprPattern) {
            ...
            FieldExprPattern.getFieldExprName = getFieldExprName, FieldExprPattern.toFieldExprEntityPattern = toFieldExprEntityPattern, 
...
!function(powerbi) {
    ...
    !function(data) {
        ...
        !function(DataViewRegression) {
            ...
            function linearRegressionTransform(sourceDataView, dataRoles, regressionDataViewMapping, objectDescriptors, objectDefinitions, colorAllocatorFactory) {
                if (sourceDataView.categorical) {
                    ...
                    if (!_.isEmpty(xColumns) && !_.isEmpty(yColumns)) {
                        var xColumnSource = xColumns[0].source, yColumnSource = yColumns[0].source, dataPointSets = extractDataPoints(xColumns, yColumns), combineSeries = !0;
                        regressionDataViewMapping.usage && regressionDataViewMapping.usage.regression && regressionDataViewMapping.usage.regression.combineSeries && sourceDataView.metadata.objects && (combineSeries = powerbi.DataViewObjects.getValue(sourceDataView.metadata.objects, regressionDataViewMapping.usage.regression.combineSeries, !0)), 
                        combineSeries && (dataPointSets = [ {
                            xValues: _.chain(dataPointSets).map(function(dataPoint) {
                                return dataPoint.xValues;
                            }).flatten().value(),
                            yValues: _.chain(dataPointSets).map(function(dataPoint) {
                                return dataPoint.yValues;
                            }).flatten().value()
                        } ]);
                        for (var categoriesByTrendLines = [], lineDefs = [], _i = 0, dataPointSets_1 = dataPointSets; _i < dataPointSets_1.length; _i++) {
                            var dataPointSet = dataPointSets_1[_i], unsortedXValues = dataPointSet.xValues, unsortedYValues = dataPointSet.yValues;
                            ...
                            if (!yDataType) return;
                            var sortedDataPointSet = sortValues(unsortedXValues, unsortedYValues), minCategoryValue_1 = sortedDataPointSet.xValues[0], maxCategoryValue_1 = sortedDataPointSet.xValues[sortedDataPointSet.xValues.length - 1], lineDef = computeRegressionLine(sortedDataPointSet.xValues, sortedDataPointSet.yValues);
                            categoriesByTrendLines.push([ minCategoryValue_1, maxCategoryValue_1 ]), lineDefs.push(lineDef);
                        }
                        for (var flattenedCategories = _.flatten(categoriesByTrendLines), valuesByTrend = [], minCategoryValue = _.min(flattenedCategories), maxCategoryValue = _.max(flattenedCategories), _a = 0, lineDefs_1 = lineDefs; _a < lineDefs_1.length; _a++) {
                            var trend = lineDefs_1[_a];
                            valuesByTrend.push([ minCategoryValue * trend.slope + trend.intercept, maxCategoryValue * trend.slope + trend.intercept ]);
                        }
                        var regressionDataView = createRegressionDataView(xColumnSource, yColumnSource, [ minCategoryValue, maxCategoryValue ], valuesByTrend, sourceDataView, regressionDataViewMapping, objectDescriptors, objectDefinitions, colorAllocatorFactory);
            ...
            function extractDataPoints(xColumn, yColumn) {
                for (var dataPoints = [], xValueArray = _.map(xColumn, function(column) {
                    return column.values;
                }), seriesYValues = _.map(yColumn, function(column) {
                    return column.values;
                }), multipleXValueColumns = xColumn.length > 1, i = 0; i < seriesYValues.length; i++) dataPoints.push({
            ...
            function createRegressionDataView(xColumnSource, yColumnSource, newCategories, newValues, sourceDataView, regressionDataViewMapping, objectDescriptors, objectDefinitions, colorAllocatorFactory) {
                ...
                seriesRole = grouped.by), yRole && seriesRole) {
                    ...
                    _b), seriesRoles = (_c = {}, _c[seriesRole] = !0, _c), groupedValues = [], valuesBySeries = [];
                    for (var index in newValues) groupedValues.push("regression" + index), valuesBySeries.push([ {
                        values: newValues[index]
                    } ]);
    ...
    !function(data) {
        ...
        function findRuleInputInColumns(columns, inputRole) {
            if (columns) for (var _i = 0, columns_6 = columns; _i < columns_6.length; _i++) {
                var column = columns_6[_i], roles = column.source.roles;
...
!function(powerbi) {
    ...
    !function(data) {
        ...
        !function(SQExprBuilder) {
            ...
            SQExprBuilder.arithmetic = arithmetic, SQExprBuilder.setAggregate = setAggregate, 
    ...
    !function(data_4) {
        ...
        var CategoricalDataViewBuilder = function() {
            ...
            return CategoricalDataViewBuilder.prototype.withCategory = function(options) {
                ...
                for (var columns = options.columns, _i = 0, columns_7 = columns; _i < columns_7.length; _i++) {
