    ...
    !function(data) {
        ...
        var DataViewCategoricalReader = function() {
            ...
            return DataViewCategoricalReader.prototype.hasCategories = function() {
                ...
                return void 0 === seriesIndex && (seriesIndex = 0), this.hasAnyValidValues ? this.grouped[seriesIndex].values[this.getMeasureIndex(roleName)] : void 0;
            ...
            }, DataViewCategoricalReader.prototype.getSeriesSource = function() {
    ...
    !function(data) {
        ...
        !function(DataViewTransform) {
            ...
            function transformDataView(prototype, objectDescriptors, roleMappings, transforms, colorAllocatorFactory, dataRoles, selectsToInclude) {
                ...
                transformed = data.DataViewConcatenateCategoricalColumns.detectAndApply(transformed, roleMappings), 
    ...
    !function(DataViewAnalysis) {
        ...
        function chooseDataViewMappings(projections, mappings, roleKindByQueryRef, objectDescriptors, objectDefinitions) {
            for (var supportedMappings = [], i = 0, len = mappings.length; len > i; i++) {
                var mapping = mappings[i], mappingConditions = mapping.conditions, requiredProperties = mapping.requiredProperties, conditionsMet = isAnyConditionMet(mappingConditions, projections, roleKindByQueryRef), allPropertiesValid = areAllPropertiesValid(requiredProperties, objectDescriptors, objectDefinitions);
                conditionsMet && allPropertiesValid && supportedMappings.push(mapping);
            }
            return ArrayExtensions.emptyToNull(supportedMappings);
        }
        function isAnyConditionMet(mappingConditions, projections, roleKindByQueryRef) {
            return _.isEmpty(mappingConditions) ? !0 : _.some(mappingConditions, function(condition) {
                return matchesCondition(projections, condition, roleKindByQueryRef);
            });
        }
        ...
        function matchesCondition(projections, condition, roleKindByQueryRef) {
            for (var conditionRoles = Object.keys(condition), i = 0, len = conditionRoles.length; len > i; i++) {
                var roleName = conditionRoles[i], isDrillable = projections[roleName] && !_.isEmpty(projections[roleName].activeProjectionRefs), roleCondition = condition[roleName], roleCount = getPropertyCount(roleName, projections, isDrillable);
...
!function(powerbi) {
    ...
    !function(data) {
        ...
        !function(DataViewConcatenateCategoricalColumns) {
            function detectAndApply(dataView, roleMappings) {
                var result = dataView, dataViewCategorical = dataView.categorical;
                if (dataViewCategorical) {
                    var concatenationSource = detectCategoricalRoleForHierarchicalGroup(dataViewCategorical, roleMappings);
                    concatenationSource && (result = applyConcatenation(dataView, concatenationSource));
                }
                return result;
            }
            function detectCategoricalRoleForHierarchicalGroup(dataViewCategorical, roleMappings) {
                var result, roleMappingForCategorical = roleMappings && 1 === roleMappings.length && roleMappings[0].categorical ? roleMappings[0] : void 0;
            ...
            function getAllRolesInCategories(categoricalRoleMapping) {
                ...
                if (categoricalRoleMapping.categories) {
                    var mappings = categoricalRoleMapping.categories.select;
                    mappings || (mappings = [ categoricalRoleMapping.categories ]);
                    for (var _i = 0; _i < mappings.length; _i++) {
                        var mapping = mappings[_i], forValue = mapping["for"];
                        if (forValue) roleNames.push(forValue["in"]); else {
                            var bindValue = mapping.bind;
                            bindValue && roleNames.push(bindValue.to);
                        }
                    }
                }
                return roleNames;
            }
            function applyConcatenation(dataView, concatenationSource) {
                var concatenatedValues = concatenateValues(concatenationSource.categories), concatenatedColumnMetadata = createConcatenatedColumnMetadata(concatenationSource.categories, concatenationSource.roleName), transformedDataView = inheritSingle(dataView);
                addToMetadata(transformedDataView, concatenatedColumnMetadata);
                var concatenatedCategoryColumn = createConcatenatedCategoryColumn(concatenationSource.categories, concatenatedColumnMetadata, concatenatedValues), dataViewCategorical = dataView.categorical, transformedCategoricalCategories = _.difference(dataViewCategorical.categories, concatenationSource.categories);
            ...
            function concatenateValues(categoryColumns) {
                for (var concatenatedValues = [], _i = 0; _i < categoryColumns.length; _i++) for (var categoryColumn = categoryColumns[_i], i = 0, len = categoryColumn.values.length; len > i; i++) {
                    var valueToAppend = categoryColumn.values && categoryColumn.values[i];
                    concatenatedValues[i] = void 0 === concatenatedValues[i] ? valueToAppend + "" : concatenatedValues[i] + " " + valueToAppend;
                }
                return concatenatedValues;
            }
            function createConcatenatedColumnMetadata(categoryColumns, roleName) {
                debug.assertNonEmpty(categoryColumns, "categoryColumns");
                for (var concatenatedDisplayName, consistentIsMeasure = categoryColumns[0].source.isMeasure, _i = 0; _i < categoryColumns.length; _i++) {
                    var categoryColumn = categoryColumns[_i], columnSource = categoryColumn.source;
                ...
                newColumnMetadata.queryName = "Concatenated Column Metadata", newColumnMetadata;
            }
            ...
            function createConcatenatedCategoryColumn(concatenationSourceColumns, columnMetadata, concatenatedValues) {
                var newCategoryColumn = {
                    source: columnMetadata,
                    values: concatenatedValues
                }, firstColumn = concatenationSourceColumns[0];
