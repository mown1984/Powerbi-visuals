...
!function(jsCommon) {
    ...
    !function(DOMConstants) {
        ...
        DOMConstants.vKeyCode = 86, DOMConstants.xKeyCode = 88, DOMConstants.yKeyCode = 89, 
        DOMConstants.zKeyCode = 90, DOMConstants.DocumentBody = "body", DOMConstants.Anchor = "a", 
        DOMConstants.EditableTextElements = ":text, textarea", DOMConstants.disabledAttributeOrValue = "disabled", 
...
!function(jsCommon) {
    ...
    !function(ArrayExtensions) {
        ...
        function isUndefinedOrEmpty(array) {
            return array && 0 !== array.length ? !1 : !0;
        }
...
!function(jsCommon) {
    ...
    !function(CssConstants) {
        ...
        CssConstants.coverValue = "cover", CssConstants.pointerValue = "pointer";
    ...
    !function(TextMeasurementService) {
        ...
        function estimateSvgTextRect(textProperties) {
            ...
            if (null == rect) {
                ...
                rect = measureSvgTextRect(estimatedTextProperties), powerbi.ephemeralStorageService.setData(propertiesKey, rect);
            }
            return rect;
        }
        ...
        function getTailoredTextOrDefault(properties, maxWidth) {
            ...
            if (0 === strLength) return properties.text;
            var width = measureSvgTextWidth(properties);
            if (maxWidth > width) return properties.text;
            for (var text = properties.text = ellipsis + properties.text, min = 1, max = text.length, i = ellipsis.length; max >= min; ) if (i = (min + max) / 2 | 0, 
            properties.text = text.substr(0, i), width = measureSvgTextWidth(properties), maxWidth > width) min = i + 1; else {
                if (!(width > maxWidth)) break;
                max = i - 1;
            }
            return properties.text = text.substr(0, i), width = measureSvgTextWidth(properties), 
...
!function(jsCommon) {
    ...
    var Utility = function() {
        ...
        return Utility.throwIfNullOrUndefined = function(value, context, methodName, parameterName) {
            ...
            throw jsCommon.Trace.error(StringExtensions.format("Throwing exception: {0}", JSON.stringify(e)), null != e.stack ? !1 : !0), 
            e;
            ...
            if (StringExtensions.isNullOrEmpty(url)) return !1;
            var match = jsCommon.RegExpExtensions.run(Utility.urlRegex, url);
            return !!match && 0 === match.index;
        }, Utility.findAllValidUrls = function(text) {
            if (StringExtensions.isNullOrEmpty(text)) return [];
            for (var matches, urlRanges = [], start = 0; null !== (matches = jsCommon.RegExpExtensions.run(Utility.urlRegex, text, start)); ) {
        ...
        Utility.urlRegex = /http[s]?:\/\/(\S)+/gi, Utility;
    }();
