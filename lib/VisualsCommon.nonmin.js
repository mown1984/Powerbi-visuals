...
!function(powerbi) {
    ...
    !function(TextMeasurementService) {
        ...
        function getTailoredTextOrDefault(textProperties, maxWidth) {
            ...
            for (var text = textProperties.text = ellipsis + textProperties.text, min = 1, max = text.length, i = ellipsis.length; max >= min; ) if (i = (min + max) / 2 | 0, 
            textProperties.text = text.substr(0, i), width = measureSvgTextWidth(textProperties), 
            maxWidth > width) min = i + 1; else {
                if (!(width > maxWidth)) break;
                max = i - 1;
            }
            return textProperties.text = text.substr(0, i), width = measureSvgTextWidth(textProperties), 
